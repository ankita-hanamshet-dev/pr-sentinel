...
C2	The hard quota is now DOLLARS, not requests. The retired GitHub Models free tier had an rpm/rpd quota; a paid API (Anthropic default) does not, so the primary governor limit is PR_SENTINEL_MAX_USD_PER_RUN (default $1.00), computed from the token ledger at per-model STANDARD rates held in settings.model_prices — never hardcoded. The governor refuses call N+1 once accrued spend reaches the cap; MAX_LLM_CALLS_PER_RUN (target ≤ 12/PR) stays a SECONDARY cap. Keep content-hash caching, diff-only analysis, hunk batching, intra-job prompt caching, and graceful degradation on exhaustion.	STANDARD rates (documentation + demo always quote these, never the promo): claude-sonnet-5 $3.00 in / $15.00 out per 1M; cache read ≈ 0.1×, write ≈ 1.25×. Sonnet's $2/$10 intro rate expires 2026-08-31 and must not appear in docs. rpm/rpd knobs remain only for the legacy github_models adapter.
C3	Cap input tokens per request at MAX_INPUT_TOKENS (default 8 000; Anthropic ceiling, ~30 KB) and MAX_OUTPUT_TOKENS (default 4 000).	Never send whole files. Send diff hunks plus a small context window. Token-count before every call and split. tiktoken accounting asserted before send.
...
Secrets — exactly one, and where inference is allowed to run
There is ONE secret: PR_SENTINEL_LLM_API_KEY (the Anthropic/TCS model key). GITHUB_TOKEN is not a secret you set — it is the Actions-provided GitHub REST key, and it is never the model key. Because the default provider is now a paid API rather than the retired GitHub Models (which authenticated inference with GITHUB_TOKEN itself), inference requires that secret.
GitHub withholds repository secrets from fork-triggered pull_request runs (C4). Therefore inference CANNOT run on the pull_request (analyze) side for fork PRs. This is why the pipeline is split by trust, not just by concern:
- Analyze (pull_request): diff extraction + HEURISTIC triage only. No model call, no secret, no write scope, no models: read. Fork-safe. Uncertain files are emitted as risk="unknown".
- Publish (workflow_run): runs in the base-repo context, so it has the secret and write scope. ALL inference lives here — the LLM triage refinement (only risk="unknown" files), the four specialist agents, and the critic. It never checks out PR head code and reads pr_number only from pr-meta.json.
The triage strategy (PR_SENTINEL_TRIAGE_STRATEGY = heuristic | llm | hybrid, default hybrid) is a config flag over an IDENTICAL TriagePlan schema: escalate to the model on uncertainty, not on every file.
Repository layout
pr-sentinel/
├── CLAUDE.md
├── .github/workflows/
...
Line-ending normalization invariant. The diff parser normalizes line endings: Hunk.lines and every added-line content are LF-normalized (trailing \r stripped). The original ending is preserved as Hunk.line_ending / FileDiff.line_ending (lf|crlf|mixed|none). Two consequences every downstream consumer inherits: (1) grounding — any comparison of a finding's evidence_quote against raw file bytes MUST normalize both sides, or a CRLF file's "foo\r" will never match the parser's "foo"; (2) the Fixer — a `suggestion` block MUST restore the file's original line_ending, or "Apply suggestion" on a CRLF file injects spurious whitespace-only changes on every touched line.
